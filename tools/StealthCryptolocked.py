...
class StealthCryptolocked(Core):
	...
	database=r"adarch_persistent/stealthcryptolocked.db"
			...
			if (mask & (Core.IN_MODIFY |
				...
				if is_trip_file(conn, filename):
						...
						if self.action & Core.LOG_EVENT:
							super().log(Core.CRITICAL, "STEALTH CRYPTOLOCKED" , "file '{}/{}' has been {}! {}".format(target, name, action, audit_info))

						if comm != "adarch": #execute action only if the event was not caused by adarch itself
							execute_action(self, "STEALTH CRYPTOLOCKED", self.action, ppid, user, filename)
